...
const PATCH_ATTR = "data-shadcn-patch";
const NODE_CSS_ATTR = "data-shadcn-node-css";
const SHELL_ATTR = "data-shadcn-shell";
...
return baseclass.extend({
  __init__() {
    ...
    if (missing.length) {
      console.warn(
        `router-shadcn: luci-base surface missing, staying MPA: ${missing.join(", ")}`,
      );
      return;
    }
    ...
    this.hostname =
      document.getElementById("sidebar")?.dataset.hostname || document.title;
    this.hookIntervals();
    this.hookListeners();
    this.hookSession();
    this.hookVisibility();

    Promise.all([ui.menu.load(), RT.require("menu-shadcn")]).then(
      ([tree, menu]) => {
        this.tree = tree;
        this.menu = menu;
  ...
  progressStart() {
    ...
    this.progressTimer = setTimeout(() => {
      ...
      this.bar = document.body.appendChild(
        E("div", { id: "shadcn-nav-progress", "aria-hidden": "true" }),
      );
  ...
  initialRoute() {
    ...
    // header.ut titles pages "<hostname> | <page title>"; keep whatever it
    // emitted before the separator so the router's titles match its own.
    const tail = current.node.title ? ` | ${_(current.node.title)}` : "";
    this.titleHead =
      tail && document.title.endsWith(tail)
        ? document.title.slice(0, -tail.length)
        : this.hostname;
    return current;
  },
  ...
  rendered(view) {
    const done = (v) =>
      ...
      (v.childElementCount === 0 && v.dataset.shadcnStarted);
  ...
  onNavigate(ev) {
    ...
    // #maincontent is the scroller, not the window, so the browser's own
    // scroll restoration never sees it: remember the departing entry's
    // offset here (currentEntry still names it during dispatch) and put it
    // back on traversal, top on push, in commit().
    const main = document.getElementById("maincontent");
    if (main && navigation.currentEntry)
      this.scrolls.set(navigation.currentEntry.key, main.scrollTop);
    ...
    const swap = () => {
      ...
      main.scrollTop =
        ev.navigationType === "traverse"
          ? (this.scrolls.get(ev.destination.key) ?? 0)
          : 0;
    };
  ...
  mountPatches(pending) {
    const registry = window.shadcn?.patches ?? {};
    ...
    for (const stem of pending) {
      ...
      if (!script) {
        ...
        el.addEventListener("load", () => {
          if (!this.live.has(stem)) window.shadcn?.patches?.[stem]?.unmount?.();
        });
  ...
  unmountPatches() {
    this.live = new Set();
    for (const stem in window.shadcn?.patches ?? {})
      window.shadcn.patches[stem].unmount?.();
  },
});
