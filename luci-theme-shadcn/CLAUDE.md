...
- `mock-pages-plugin` — serves saved page snapshots at `/mocks/` against the live theme (see **Mock Pages** below)
...
- **Navigate**: `scripts/mock-nav.client.js` (injected into every mock) takes over clicks on the snapshot's own `/cgi-bin/luci/…` links, matching them to snapshots by `data-page` and jumping in place — an app's own tab bar works as it does on the device. Uncaptured targets are blocked with a hint naming the missing snapshot. A bottom-left floating switcher lists all snapshots and cycles with <kbd>[</kbd>/<kbd>]</kbd>.
